...
export function PaymentsManager() {
    ...
    const loadPayments = async (storeId: string) => {
        ...
        try {
            setLoading(true)
            const ordersResponse = await shoppingInventoryAPI.getOrders(token, storeId)
            ...
            const paymentTransactions: PaymentTransaction[] = ordersResponse.orders
                ...
                .map(order => ({
                    ...
                    payment_method: (order.payment_method as keyof typeof PAYMENT_METHOD_ICONS) || 'mobile_money',
    ...
    const getPaymentStats = () => {
        ...
        const successRate = totalTransactions > 0
            ? (filteredTransactions.filter(t => t.status === 'paid').length / totalTransactions) * 100
            : 0

        return { totalTransactions, totalRevenue, totalFees, netRevenue, successRate }
    }
                    ...
                    {loading ? (
                        ...
                        <div className="space-y-4">
                            {filteredTransactions.map((transaction) => {
                                const PaymentIcon = PAYMENT_METHOD_ICONS[transaction.payment_method]
                                return (
                                    <div
                                        key={transaction.id}
                                                ...
                                                <div className="space-y-1">
                                                    ...
                                                    {transaction.status === 'paid' && (
                                                        <div className="flex items-center gap-4 text-xs text-muted-foreground">
                                                            <span>Fee: UGX {transaction.transaction_fee.toLocaleString()}</span>
                                                            <span>Net: UGX {transaction.net_amount.toLocaleString()}</span>
                                                        </div>
                                                    )}
                                                </div>
                                            </div>
