...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const storeAuth = async (authData: AuthResponse) => {
    if (typeof window !== "undefined") {
      ...
      document.cookie = `piaxis_auth_token=${
        authData.access_token
      }; path=/; max-age=${30 * 24 * 60 * 60}`; // 30 days
      document.cookie = `piaxis_refresh_token=${
  ...
  const clearAuth = () => {
    if (typeof window !== "undefined") {
      ...
      // Also clear cookies
      document.cookie =
        "piaxis_auth_token=; path=/; expires=Thu, 01 Jan 1970 00:00:00 GMT";
      document.cookie =
        "piaxis_refresh_token=; path=/; expires=Thu, 01 Jan 1970 00:00:00 GMT";
    }
    setToken(null);
    setUser(null);
  };
