...
const API_BASE_URL = process.env.NEXT_PUBLIC_API_BASE_URL || "https://piaxe.jettts.com"
...
class WalletAPI {
  ...
  async getTransactions(token: string, params?: {
    ...
    try {
      ...
      const url = `${API_BASE_URL}/wallet/transactions${searchParams.toString() ? '?' + searchParams.toString() : ''}`
  ...
  async getBalance(token: string, currency?: string): Promise<{ balance: string; currency: string }> {
    try {
